...
[Releases](https://github.com/nlink-jp/slack-router/releases) からお使いの環境に合わせた zip をダウンロードして展開します。
...
```bash
git clone https://github.com/nlink-jp/slack-router.git
cd slack-router
make build
```
...
ルーターはワーカーの終了コードによって通知の有無を切り替えます。

| 終了パターン | ExitCode | ルーターによる通知 |
|---|---|---|
| 正常終了 | `0` | なし |
| `exit 1` / `exit 2` など | `> 0` | **なし** — スクリプトが意図的に終了したとみなし、スクリプト自身が `response_url` でユーザーに通知済みと期待する |
| シグナルで強制終了（OOM killer・外部からの SIGKILL 等） | `< 0` | **あり** — スクリプトが通知を送れていないため、ルーターが `error_message` を送信 |
| ワーカー起動失敗（`cmd.Start` 失敗・stdin エラー等） | — | **あり** — `error_message` を送信 |

> **スクリプトの実装規約:**
> - 処理成功・失敗を問わず、ユーザーへの返信は `response_url` 経由でスクリプト自身が行う
> - 返信を送ったうえで処理を終えるときは必ず `exit 0` で終了する
> - `exit 1` 以上で終了した場合、ルーターは通知を送らないため、ユーザーへの説明はスクリプト内で完結させること
